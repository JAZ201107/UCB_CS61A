...
def closer_city(lat, lon, city_a, city_b):
    ...
    distance_to_city_a = distance(temp_city, city_a)
    distance_to_city_b = distance(temp_city, city_b)
...
def count_palindromes(L):
    ...
    return len(
        tuple(
            filter(
                lambda word: word.lower() == word.lower()[::-1], 
                L
                )
            )
        )
...
def coords(fn, seq, lower, upper):
    ...
    "*** YOUR CODE HERE ***"
    return [
        [x, fn(x)] for x in seq if lower <= fn(x) <= upper
        ]
